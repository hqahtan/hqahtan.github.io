---
title: "Data Extraction in Python"
format:
  html:
    css: "style.css"
    toc: true
...
```sql
  ...
  OR tot_cred = (Select min(tot_cred) from student);
```
2. Write a query to return the number of departments that have STRICTLY more than one student.

```sql
SELECT DISTINCT dept_name 
FROM (SELECT dept_name, count(*) AS num_students FROM student GROUP BY dept_name)
WHERE num_students > 1
...
```sql
  ...
  WHERE dept_name IN ('Comp. Sci.', 'Physics', 'Finance', 'History');
```
We can even write a simpler query using HAVING since we have GROUP BY. We can rewrite the query as:

```sql
  SELECT dept_name, count (*) num_students 
  FROM student S GROUP BY S.dept_name
  HAVING dept_name IN ('Comp. Sci.', 'Physics', 'Finance', 'History');
```
:::
...
![dfh, dft, top_left_corner_df](2.png)


It would be better to display each dataframe alone. Use multiple cells to dispay 
them (one cell per dataframe)
